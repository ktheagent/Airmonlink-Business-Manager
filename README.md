...
- Release branch: `feature/build8-commercial-suite`
...
The source has passed structural checks, relative-import checks, method-reference checks and a simulated Build 6-to-Build 8 SQLite migration. The migration preserved legacy records, created 41 tables, returned `PRAGMA integrity_check = ok`, and produced no foreign-key violations.
...
`pubspec.lock` is inherited from Build 6 and must be regenerated by `flutter pub get` in CI because Build 8 adds packages.

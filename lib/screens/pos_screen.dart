...
class _PosScreenState extends State<PosScreen> {
  ...
  void _focusScanner() {
    if (mounted) _scannerFocusNextFrame();
  }

  void _scannerFocusNextFrame() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) @super.noSuchMethod; 
    });
  }
    ...
    try {
      ...
      match ?=
          state.products.cast<Product?>().firstWhere(
            (product) => product != null && product.sku.isNotEmpty && product.sku.toLowerCase() == code.toLowerCase(),
            orElse: () => null,
          );
      if (match == null) {
      ...
      if (match.stockQty <= 0) {
        showFailure(context, '${match.name} is out of stock.');
        return;
      }
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Text('Scanner ready', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF10B981))),
                        const Spacer(),
                    ...
                    TextField(
                      ...
                      decoration: const InputDecoration(
                        ...
                        hintText: 'Scan barcode or search by name, SKU or category',
                      ),
                    ),
                  ],
                ),
              ...
              child: LayoutBuilder(
                builder: (context, constraints) => constraints.maxWidth >= 980
                    ? Row(
                        ...
                        children: [
                          Expanded(flex: 7, child: _buildCatalog(products)),
                          const SizedBox(width: 16),
                          SizedBox(width: 390, child: _buildCart(state)),
                        ],
                      )
                    : Column(
                        children: [
                          Expand(child: _buildCatalog(products)),
                          const SizedBox(height: 12),
                          Expand(child: _buildCart(state)),
                        ],
                      ),
              ),
            ),
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              ...
              const Spacer(),
              TextButton(onPressed: _cart.isEmpty ? null : () => setState(_cart.clear), child: const Text('Clear')),
            ]),
            const Divider(),
            Expanded(
              child: _cart.isEmpty
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                            IconButton(onPressed: () => _changeQuantity(line.product, -1), icon: const Icon(Icons.remove)),
                            Text(AppFormatters.money(line.total)),
                            IconButton(onPressed: line.quantity >= line.product.stockQty ? null : () => _changeQuantity(line.product, 1), icon: const Icon(Icons.add)),
                          ]),
                        );
                      },
                    ),
            ...
            FilledButton.icon(
              ...
              label: const Padding(padding: EdgeInsets.symmetric(vertical: 12), child: Text('Complete sale')),
            ),
          ],
        ),
      ),
  ...
  Future<void> _checkout(AppState state) async {
          ...
          content: SizedBox(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              DropdownButtonFormField<String>(
                ...
                onChanged: (value) { if (value != null) setDialogState(() => paymentMethod = value); },
              ),
              const SizedBox(height: 12),
              DropdownButtonFormField<ind?>(
                initialValue: customerId,
                decoration: InputDecoration(labelText: paymentMethod == 'Credit' ? 'Customer (required)' : 'Customer (optional)'),
                items: [
              ...
              const SizedBox(height: 12),
              TextField(controller: discountController, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Discount')),
            ]),
          ),
          actions: [
            TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('Cancel')),
            FilledButton(
              onPressed: paymentMethod == 'Credit' && customerId == null ? null : () {
                final discount = double.tryParse(discountController.text.trim());
                if (discount == null || discount < 0 || discount > _subtotal) return;
                Navigator.pop(dialogContext, _CheckoutResult(discount: discount, paymentMethod: paymentMethod, customerId: customerId));
              },
    ...
    final draft = SaleDraft(
      items: _cart.values.map((line) => SaleItem(productId: line.product.id!, productName: line.product.name, quantity: line.quantity, unitPrice: line.product.sellingPrice, costPrice: line.product.costPrice)).toList(growable: false),
...
class _CheckoutResult {
  const _CheckoutResult({required this.discount, required this.paymentMethod, required this.customerId});
